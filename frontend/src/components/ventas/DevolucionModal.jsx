  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">Venta #{venta.numero_venta}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        {/* Advertencia */}
        <div className="p-6 bg-yellow-50 border-l-4 border-yellow-400 flex gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-yellow-700 mt-1">
              Los productos volverán al stock disponible y el monto se restará del cuadro de caja del vendedor.
            </p>
          </div>
        </div>
        ...
        <div className="p-6 border-b">
          <h3 className="font-semibold text-gray-900 mb-3">Detalles de la Venta</h3>
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            <div>
              ...
              <p className="font-medium text-lg">S/ {Number(venta.total || 0).toFixed(2)}</p>
            </div>
            <div>
          ...
          {venta.detalles && venta.detalles.length > 0 && (
            <div className="mt-4">
              <span className="text-gray-600 text-sm">Productos a devolver:</span>
                ...
                {venta.detalles.map((detalle, index) => (
                  <div key={index} className="flex justify-between items-center py-2 border-b border-gray-200 last:border-0">
                    <div>
                      ...
                      <p className="text-xs text-gray-600">Cantidad: {detalle.cantidad}</p>
                    </div>
                    <p className="text-sm font-medium">S/ {Number(detalle.subtotal).toFixed(2)}</p>
                  </div>
                ))}
              </div>
            </div>
